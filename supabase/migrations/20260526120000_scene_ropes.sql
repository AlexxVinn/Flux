-- Ropes: logical cable constraints between two bodies (segments are client-only).
...
begin
  ...
  loop
    if coalesce(elem->>'entityKind', '') not in ('wall', 'floor', 'ropeSegment') then
      body_count := body_count + 1;
    end if;
  end loop;
  spring_count := jsonb_array_length(coalesce(p_snap->'springs', '[]'::jsonb));
  rope_count := jsonb_array_length(coalesce(p_snap->'ropes', '[]'::jsonb));
  return body_count + spring_count + rope_count;
end;
$$;
...
begin
  ...
  if v_type in ('entity.add.body', 'entity.add.spring', 'entity.add.rope', 'entity.remove', 'scene.replace') then
    return true;
  end if;
