-- Per-entity lock flag: excluded from marquee/group select; direct pick still works.
...
declare
  ...
  v_allowed text[] := array[
    ...
    'isStatic','visible','locked','showTrajectory','displayName','label','gravityScale','sleepThreshold','isSleeping'
  ];
  v_sp_allowed text[] := array[
    'stiffness','damping','length','elasticConstantNnPerM','visible','locked','displayName'
  ];
  v_rope_allowed text[] := array['linkStiffness','linkDamping','segmentCount','visible','locked','displayName'];
  v_mk_allowed text[] := array['points','text','visible','locked','displayName','measureUnit'];
  v_child jsonb;
begin
